<div class="register">
    <form action="../../app/reg_form.php" method="POST">
        ...
        <input type="text" id="name" name="name" value="<?php if($_SESSION['mail']) { echo $_SESSION['mail']; }; ?>" class="register-field" required pattern="[A-Za-z-,]{2,15}"><br>
        <label for="email">Email:</label><br>
        <input type="email" id="email" name="email" value="<?php if($_SESSION['email']) { echo $_SESSION['email']; }; ?>" class="register-field" required pattern="(\w\.?)+@[\w\.-]+\.\w{2,}"><br>
